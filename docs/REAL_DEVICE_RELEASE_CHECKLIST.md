...
| 記号の描画 | 記号メニューで基本4種・減目7種の計11記号が同梱資産から正しく描画された |

未了は次の3点。

- 26記号すべての目視。11記号まで確認した。残りはスクロールが必要だが、機内モード中はXCUITestが使えず自動化できない（後述）。26記号の定義とID、全記号のCanvas・PNG・PDF描画は自動テストで担保している。
- 機内モード前後の通信監視。
- 機内モードでの新規起動（署名制約）。
...
- [ ] タッチ描画、2本指パン／ピンチ、マウス／トラックパッドを確認する。（2026-09-22、iPad Air 第5世代でタッチ描画・2本指パン・ピンチ・消去・範囲選択と貼り付けを確認。2本指ジェスチャで記号が入る不具合を発見し修正した。マウス／トラックパッドは機材が無く未実施）
