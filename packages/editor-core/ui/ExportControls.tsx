...
import { pdfPageCount } from '../export/pdfLayout';

export interface ExportControlsProps {
  board: Board;
  onPng: (size: number) => void;
  onPdf: (layout: 'single' | 'tiled', orientation: 'portrait' | 'landscape', millimeters: number) => void;
...
export function ExportControls({ board, onPng, onPdf, onBackup, onRestore, backupNote }: ExportControlsProps) {
  ...
  const pages = pdfPageCount(board.rows, board.cols, { layout, orientation, cellMillimeters: millimeters });
