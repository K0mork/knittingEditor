...
- [ ] Web版コミット`8d33857`を初期同期元として記録する
- [ ] カタログv3の26記号と`glyphs.ts`を取り込む
- [ ] 永続ID 1〜26のスナップショットテストを追加する
- [ ] React UI、Board、Canvas、blocks、export、storageをアプリ用ビルドへ組み込む
- [ ] Web版専用のSEO、CNAME、サイトマップ、分析処理を除外する
- [ ] `localStorage`旧版移行をアプリビルドで無効化する
- [ ] 使い方ページをオフライン資産として同梱する
- [ ] Web版由来の単体テストをアプリ側で実行できるようにする
...
- [ ] scene phase変化をJavaScriptへ通知する
- [ ] バックグラウンド移行時に保留中の保存をflushする
- [ ] 保存完了、保存失敗、復旧案内を定義する
