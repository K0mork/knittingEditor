...
|---|---|---|---|
| 保存時間 | 失敗・ハングなし、実測値を記録 | 1.1〜2.2秒（2026-09-22、iPhone 17 / iOS 27.0） | 未実施 |
| 復元時間 | 失敗・ハングなし、実測値を記録 | 2.6〜2.7秒（アプリ終了から1000×1000と記号の復元まで） | 未実施 |
| ピークメモリ | 警告・強制終了なし、実測値を記録 | アプリ本体プロセスはフットプリント16.5 MiB／Resident 52 MiB。WebKitコンテンツプロセスは未測定（下記） | 未実施 |
| PNG/PDF出力 | エラー表示が仕様どおり | 未実施 | 未実施 |

1000×1000盤面のその他の実測値（iPhone 17 / iOS 27.0）。20×20からのリサイズ1.8秒、記号配置の反映1.7秒。ハング・クラッシュ・メモリ警告は発生しなかった。

盤面データ（`Uint32Array`と`Int32Array`で各約4MB）はアプリ本体ではなくWebKitのコンテンツプロセスにあるため、本体プロセスの値はピークメモリの代表値にならない。コマンドラインからの測定は次の理由で断念した。Allocationsテンプレートは起動時の注入が必要でattachでは使えず、Activity Monitorは対象PIDの指定が要るが`com.apple.WebKit.WebContent`のPIDはアプリ再起動のたびに変わる。
