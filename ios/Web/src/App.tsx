...
function ExportControls({ board, onPng, onPdf, onBackup, onRestore }: {
  ...
  return <div className="export-controls">
    ...
    <h3>バックアップ</h3><div className="button-grid"><button onClick={() => onBackup(false)}>この編み図</button><button onClick={() => onBackup(true)}>全データ</button><button onClick={onRestore}>復元</button></div><p>アプリを削除すると端末内のデータも消えます。定期的にバックアップを保存してください。</p>
  </div>;
}
