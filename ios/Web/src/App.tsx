...
import { useClipboardShortcuts, useModalFocus, useNotifier, usePanelFocus } from '@knitting-editor/editor-core/ui/hooks';
...
export default function App() {
  ...
  const {
    ...
    onSaveError: (_error, trigger) => notify(trigger === 'background'
      ? 'バックグラウンド移行前の自動保存に失敗しました。バックアップを保存してください。'
      : trigger === 'autosave'
  ...
  return <div className="app-shell">
    ...
    {panel && <aside ref={panelRef} id="app-drawer" className="drawer" aria-labelledby="app-drawer-title">
      ...
      {panel === 'export' && <ExportControls board={board} onPng={runPngExport} onPdf={runPdfExport} onBackup={backup}
        onRestore={() => { if (!requestNativeBackupOpen()) fileInputRef.current?.click(); }}
