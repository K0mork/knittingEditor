# Development Log（凍結アーカイブ）

**このファイルは、Web版リポジトリへ統合する前のアプリ履歴の記録です。追記しません。**
統合後の記録先はリポジトリルートの`DEVELOPMENT_LOG.md`だけです。iOS側の変更もそちらへ書きます。

以下は当時の記録方針で、内容は当時のまま残します。ユーザーに見える機能、挙動、データ形式、移行、テスト・ビルド設定、配布設定の変更を、新しいものから順に記録します。文書だけの変更でも、方針・仕様・運用規則を変えた場合は記録します。
...
## 2026-09-22: 1000×1000盤面の時間を実機測定しM2の手順を整理

- 測定: iPhone 17（iOS 27.0）実機で1000×1000盤面の時間を実測した。20×20からのリサイズ1.8秒、記号配置の反映1.7秒、保存完了まで1.1〜2.2秒、アプリ終了から再起動して1000×1000と記号を復元するまで2.6〜2.7秒。ハング、クラッシュ、メモリ警告は発生しなかった。
- 追加: 測定用のXCUITest`testLargeBoardSavesAndRestores`を追加した。`TEST_RUNNER_KNITTING_EDITOR_LARGE_BOARD=1`を付けたときだけ実行する。盤面サイズの数値欄はXCUITestからの入力が安定しない（タップ後のキャレット位置が実行ごとに変わり、`20`が`01000`や`100020`になる）ため、盤面を手で1000×1000にしてから実行する前提とした。単純タップ、3回タップ、長押しからの全選択のいずれも安定しなかった。文字入力ダイアログのほうは3回タップで安定している。
- 未測定: WebKitコンテンツプロセスのピークメモリ。盤面データはアプリ本体ではなくコンテンツプロセスにあるため、本体プロセスの16.5 MiBは代表値にならない。Allocationsは起動時の注入が必要でattachでは使えず、Activity Monitorは対象PIDの指定が要るが`com.apple.WebKit.WebContent`のPIDが再起動のたびに変わるため、コマンドラインからの取得を断念した。Instruments.appを手で使う手順を実機チェックリストへ記載した。
- 主なファイル: `UITests/KnittingEditorUITests/KnittingEditorUITests.swift`、`docs/REAL_DEVICE_RELEASE_CHECKLIST.md`
- 実行コマンドと結果: iPhone 17実機で`testLargeBoardSavesAndRestores`が13.1秒で成功。
- 未実施: iPad側の同測定。PNG／PDF出力時のメモリとエラー表示。
- 配布影響: アプリの実装は変更していない。測定とテスト、手順の記録のみ。
...
- 実測: 端末内IndexedDBを有線で読み出し、13:09:56と13:10:28に編み図が作成され、`（復元）`付きの編み図が1件増えたことを確認した。アプリのプロセスは継続し、クラッシュログは無い。
- これでM3の取り込み経路が両方成立した。アプリ内「復元」のDocument Picker経路と、Files・AirDrop等からの`onOpenURL`経路である。
- 主なファイル: `docs/REAL_DEVICE_RELEASE_CHECKLIST.md`
- 未実施: iPhone側の同じ往復（PNG／PDF保存と復元）。上書き、存在しないファイルの扱い。VoiceOverのフォーカス順。機内モード前後の通信監視。
- 配布影響: 実装は変更していない。実機確認の記録のみ。

## 2026-09-22: 2本指操作の誤描画を修正し機内モード起動の記録を訂正

- 修正1（実機で発見）: ほぼ同時に2本指で触れても、先に触れた指の位置へ記号が置かれていた。指が触れた瞬間に記号を確定していたため、2本目が触れる前に1本目の描画が走っていた。1本指のタップを指を離すまで保留し、2本目が触れた時点で取り消すようにした。ドラッグ描画は最初の`pointermove`で開始セルごと描くため従来どおり。消去も同様に保留する。貼り付けはプレビューが出ている位置で確定するようになった。
- 回帰テスト: XCUITestへ`testTwoFingerGestureDoesNotDrawOnBoard`を追加した。2本指ピンチを2回行っても記号数が0のままであること、そのあと1本指タップでは記号を置けることを検査する。iPhone 16 Simulatorで、修正前は16.9秒で失敗し、修正後は21.3秒で成功することを確認した。
- 訂正: 「無料Personal Teamでは機内モードでアプリを起動できない」という前日の記録は誤りだった。利用者が機内モード中にアプリを終了し、ホーム画面のアイコンから起動できることをiPad Air 第5世代で確認した。できないのは開発ツール経由の導入と起動であり、`xcodebuild test`は再インストールを伴うため証明書の検証で失敗し、`devicectl device process launch`も拒否される。機内モードでの自動テストだけが実行できない。実機チェックリストのM0「機内モードを有効にしてアプリを新規起動できる」を完了にした。
- CI修正: iPad (10th generation)のCIで`testEditAndRelaunchRestoresLocalDocument`が失敗した。起動直後のWebViewが15秒以内に現れず、後続の待機も連鎖して120秒の実行上限に達していた。手元のSimulatorと実機では再現しない。起動直後の要素待ちの上限を45秒へ広げた（42箇所）。待機は要素が現れ次第終わるため成功時の所要時間は変わらず、停止したテストはCIの実行時間上限が従来どおり打ち切る。
...
- 検証結果: 可変ウィンドウ（584x861、画面1373x954）で、主要操作、盤面・ブロック・保存パネルの開閉、ダイアログのキーボード入力、盤面への描画がすべて成立した。
- 偽の成功の防止: 最初の実装では全画面のまま成功していた。原因は、直前に入れた`setUp`の向き強制がiPadのウィンドウ配置を全画面へ戻していたことと、ウィンドウの大きさを検査していなかったこと。手動ウィンドウ実行では向きに触れないようにし、ウィンドウが画面より小さいことを先に検査するようにした。配置が失われた状態では`window=... screen=...`を示して失敗する。
- 盤面タップの判定: 既存の記号が残った盤面では、中央をタップしても記号数が変わらない場合がある（複数セル記号を置き換えると増減が相殺されるため）。テスト内で空の編み図を新規作成してから描画を確認する形にして、盤面の状態に依存しないようにした。
- シートを閉じる処理の強化: iPadの「×」（label `Cancel`）を押したあと実際に閉じたかを確認し、閉じていなければ下スワイプへ落とすようにした。可変ウィンドウ検証の直後に実行したフルスイートで`testBackupExportSheetDismissesBackToEditor`が1度失敗した（単独実行では成功）。ウィンドウ状態によって押せるかどうかが変わるため、押下と閉じ方の両方を試す形へ変更した。
- 主なファイル: `UITests/KnittingEditorUITests/KnittingEditorUITests.swift`、`docs/REAL_DEVICE_RELEASE_CHECKLIST.md`
- 実行コマンドと結果: iPad Air 第5世代 実機で手動ウィンドウ検証が成功（30.6秒）。iPad実機のフルスイートは単体22件・XCUITest 11件成功（2件skip、213.5秒）。iPad (10th generation) Simulatorで単体22件・XCUITest 11件成功（190.2秒）。iPhone 16 Simulatorで単体22件・XCUITest 11件成功（201.2秒）。
- 未実施: Split Viewでの同確認（今回は可変ウィンドウのみ）。iPhone実機での再確認（取り外し中）。Apple Pencil、タッチ・ピンチ、VoiceOver、Files実保存とAirDrop、機内モード、1000x1000盤面の計測、iPad版スクリーンショット。
- 配布影響: アプリの実装は変更していない。テストと検証手順のみ。
...
- 根拠: 段番号は下から数えるため、追加・削除のどちらも上端で行うのが「上に段／上の段」ボタンと一致し、往復で編み始めを失わない。ユーザーの指示によりアプリ側を先に修正する。
- 主なファイル: `Web/src/App.tsx`、`Web/src/model/Board.test.ts`、`docs/WEB_SYNC.md`
- 注意: これはWeb版`8d33857`と同一だった挙動からの意図的な差分である。`scripts/sync-web-source.sh`は`Web/src/`を`rsync --delete`で上書きするため、`docs/WEB_SYNC.md`のアプリ専用差分へ記録した。同時に、先の点検で入れた他のWeb共通コード修正も同じ節へ記録した。Web版への反映は別途判断する。
- テスト: `Web/src/model/Board.test.ts`へ、段数を増やして戻しても最下段と最上段の記号が残ることを検査するケースを追加した。
- 実行コマンド: `npx vitest run`成功（7 files、43 tests）、`npm run typecheck`成功、`npm run build`成功。
- 実UI確認: `vite preview`の製品ビルドをブラウザで開き、20段盤面の段1（最下段）と段20（最上段）へ表目を置いてから、盤面設定で25段へ変更し20段へ戻した。盤面のアクセシブルな名前は「記号2個」のままで、両方の記号が元の段へ残ることを画面でも確認した。修正前は段1の記号が削除される操作である。
- 未実施: iOS Simulator・実機での同操作の目視確認は次のCI実行と実機チェックリストに残す。Web版本体への同修正。
- 配布影響: 保存形式、記号ID、`.knit`互換は変更しない。既存データの再解釈もない。段数変更操作の結果だけが変わる。
