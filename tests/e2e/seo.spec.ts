...
type StructuredDataNode = { '@type': string; url?: string; isAccessibleForFree?: boolean; offers?: { price?: string; priceCurrency?: string } };
...
test('exposes search and sharing metadata on the editor page', async ({ page }) => {
  await page.goto('/');
  await expect(page).toHaveTitle('棒針編み図エディタ｜無料・ブラウザで編み図作成');
