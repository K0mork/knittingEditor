...
- デプロイ影響: Web資産は再ビルドされるが画面と挙動は変わらない。`main`へのマージ後に`CI and deploy Pages`の全ジョブと`deploy`、`https://knittingeditor.com/`での編集・PNG/PDF・`.knit`入出力を確認する。TestFlight／App Storeへの影響は次回ビルドまでなし。
