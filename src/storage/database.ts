...
export async function migrateLegacyData(): Promise<boolean> {
  ...
  if (!raw) { await setSetting('legacyMigrationComplete', true); return false; }
  try {
    const legacy = JSON.parse(raw) as { numRows?: number; numCols?: number; grid?: Array<Array<{ type?: string; color?: string; multi?: number; verticalSpan?: number; isContinuation?: boolean; isContinuationVertical?: boolean }>> };
